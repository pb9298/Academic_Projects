'''
    Authors: Sneha Srinivas(1001667832)
             Pratik Ramesh Barhate(1001649826)
    In this project we have implemented rigorous 2PL with wound wait protocol. 
    Transaction table keeps track of the Transaction and its states and phases.
    Each transaction is timestamped to schedule on first come first serve basis(following wound wait protocol) 
...
def read_item(line):
    ...
    if current_transaction['phase'] == True:# Phase of the transaction is true when it is in growing phase(Wherein read, write , upgrade is done and the dataitems can only acquire the locks and none cane be released or downgraded)
        if item_id in lock_table: # Check if there is entry of item id in the lock table
            ...
            if current_lock['lock_status'] == "read_lock":                             
                ...
                if len(current_lock['locking_transactions'])==0: # Before upgrading an item lock status from read lock to write lock we have to check if there are multiple read locks on the particular item                
                    ...
                    #print(current_lock ['locking_transactions'])
                    t = current_lock['locking_transactions'][0]# If the item is not locked at all by that transaction then check for the priority of conflicting  transaction based on wound wait protocol 
...
def write_item(line):
    ...
    if transaction_table[transaction_id]['phase'] == True:# Phase of the transaction is true when it is in growing phase(Wherein read, write , upgrade is done and the dataitems can only acquire the locks and none cane be released or downgraded)
        if item_id in lock_table:# Check if there is entry of item id in the lock table
            current_lock = lock_table[item_id]# Instantiating the current lock to the corresponding item id in the lock table
            ...
            if current_lock['lock_status'] == "read_lock":                 
                if (len(t_list)==1 and t_list[0] == transaction_id):#Only one transaction can hold this item with read lock inorder to directly upgrade to the write lock
                    ...
                    print("\nExecuting " + line + "\nChanges in Tables: Found multiple read locks on the item " + str(item_id) + " for this transaction " + str(transaction_id) + ". The operations on the transactions are performed based on the result of wound-wait protocol.")
                    for t1 in t_list:
                        protocol = woundwait(t1,transaction_id,operation,item_id,line)#check the timestamp of the current transaction against all the transactions in the list so as to abort or terminate one among them using wound wait protocol
                        if protocol == "aborted":
                ...
                if len(current_lock['locking_transactions'])==0: # Before upgrading an item lock status from read lock to write lock we have to check if there are multiple read locks on the particular item                
                    ...
                    current_transaction['trans_item'].append(item_id)
                    print("\nExecuting " + line + "\nChanges in Tables: The item " + str(item_id) + "'s lock status for the transaction " + str(transaction_id) + " is a blind write to 'Write lock' in the lock table")
                ...
                '''
                    ...
                    t = t_list[0] # Since it already has write lock on the item the wound wait algorithm has to be performed to know which transaction should be granted write lock
                    woundwait(t,transaction_id,operation,item_id,line)
                '''
...
def end_transaction(t,t_state,line):
    try:
        ...
        for d_item in current_transaction['trans_item']:
            ...
            if op1['operation'] == "r": #checking  if the waiting operation is a read operation
                if current_lock['lock_status'] == status_WL:#checking if it has conflicting write lock
                    ...
                    if nxt_op['operation'] == "e":
                        transaction_table[first_waiting_trans]['list_of_waitingoperations'].pop(0)# Previous transaction is committed and the next operation will be in the queue
                 ...
                 #Iterating through the list of waiting transactions 
                    for t in waiting_t_list:
                        ...
                        t2 = t1['list_of_waitingoperations'][0] # Gets the first waiting operation 
                        if t2['operation'] == "r":
                            ...
                            nxt_op = transaction_table[first_waiting_trans]['list_of_waitingoperations'][0] # popping the next waiting operation
                            if nxt_op['operation'] == "e":
                                transaction_table[first_waiting_trans]['list_of_waitingoperations'].pop(0)# Previous transaction is committed and the next operation will be in the queue
                                print("\n Executing e" + str(first_waiting_trans) + " \nChanges in Tables: The Transaction " + str(first_waiting_trans) + " is committed and the next waiting item to be unlocked: " + str(transaction_table[first_waiting_trans]['trans_item']))
                                end_transaction(first_waiting_trans, t_committed, line) # End transaction is recursively called since next operation is end_transaction

            elif op1['operation'] == "w":#checking  if the waiting operation is a write operation
                if current_lock['lock_status'] == status_RL:#checking if it has conflicting read lock
                    if len(t_list)==0 or ((len(t_list))==1 and t_list[0] == first_waiting_trans ): # since we are upgrading the current item lock from the read lock to write lock it has to be either 0 or 1 transaction holding it
                        ...
                        trans1['trans_state'] = t_active# Updating the transaction id's state to active in the transaction table 
                        nxt_op2 = transaction_table[first_waiting_trans]['list_of_waitingoperations'][0] # pops the next waiting operation
                        if nxt_op2['operation'] == "e":
                            transaction_table[first_waiting_trans]['list_of_waitingoperations'].pop(0)# Previous transaction is committed and the next operation will be in the queue
                            print("\n Executing e" + str(first_waiting_trans) + " \nChanges in Tables: The Transaction " + str(first_waiting_trans) + " is committed and the next waiting item to be unlocked: " + str(transaction_table[first_waiting_trans]['trans_item']))
                            end_transaction(first_waiting_trans, t_committed, line)# End transaction is recursively called since next operation is end_transaction
                    elif (len(t_list) > 0): # If the list has more locking transactions then the next possible transaction to be performed is decided by wound wait protocol
                        for t1 in t_list:
                            protocols = woundwait(t1,first_waiting_trans,op1['operation'],op1['item_id'],line)
                    ...
                    if nxt_op2['operation'] == "e": # pops the next waiting operation
                        print("\nExecuting e"  + str(first_waiting_trans) + " \nChanges in Tables: The Transaction " + str(first_waiting_trans) + " is committed and the next waiting item to be unlocked: " + str(transaction_table[first_waiting_trans]['trans_item']))
...
'''
    ...
    If TS(Ti) < TS(Tj) − that is Ti, which is requesting a conflicting lock, is older than Tj − then Tj is wounded(Aborted).
    If TS(Ti) > TS(tj) − that is Ti is younger than Tj − then Ti waits(gets blocked).
    This scheme allows the yonger transaction to wait but kills the older one.
'''
def woundwait(t1,t2,operation,item_id,line):
    ...
    TS1 = Trans1['Timestamp'] #old
    TS2 = Trans2['Timestamp'] #new

    if Trans1 == Trans2:
        return "Both Transactions are same"
    if Trans2['trans_state'] != t_blocked or Trans2['trans_state'] != t_aborted: # the conflicting transaction should also be active in order to implement wound wait protocol
        if TS2 < TS1: # If TS2 is older(greater) than TS1, TS1 is wounded(aborted)
            print("\nExecuting " + line + "\nChanges in Tables: Transaction "+ str(t1) + " is aborted, and the subsequent operations are disregarded")
            end_transaction(t1,t_aborted,line)# End transaction is recursively called since the TS1 dies and restarts later with the same timestamp
...
with open("input7.txt", "r") as file: #It is good practice to use the with keyword when dealing with file objects. This has the advantage that the file is properly closed after its suite finishes, even if an exception is raised on the way. It is also much shorter than writing equivalent try-finally
                ...
                if transaction_table[transaction_id]['trans_state'] == 'blocked': # If the transaction is blocked it has to be updated in waiting transaction and waiting operation's list
                    if item_id not in lock_table:
                        if operation == "r":
                            ...
                            lock_table[item_id] = {'lock_status': status_WL, 'locking_transactions': [transaction_id], 'list_of_transactions_waiting': []}
                            #transaction_table[transaction_id]['trans_item'].append(item_id)
                    transaction_table[transaction_id]['list_of_waitingoperations'].append({'operation':operation, 'item_id':item_id}) # The current operatrion is newly added to the list of waiting operations in the transaction table  
